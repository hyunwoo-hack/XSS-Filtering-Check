export default class Input_Parser{
    ...
    param_keys(url){
        ...
        return key_array.length > 0 ? key_array : null;
    }
    ...
    host_extractor(url){
        let word_list = url.split("/");
        let protocol  = word_list[0];
        let domain    = word_list[2];
    ...
    path_calc(path, num){
        ...
        let count = 0;
        for(;;){
            re_path = re_path.slice(-1);
            if(re_path.endsWith("/")){count += 1;}
            if(count == num){return re_path;} 
        }
    }
}