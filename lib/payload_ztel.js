...
export default class Payload_Ztel extends Advance_Filter_Finder{
    ...
    #impl_attack_checker(response, payload, pay_type){
        ...
        if(attack_res[1] == null || attack_res[1][0] == null || attack_res[1][0] == " ")return false;
        if(attack_res[1][0].replace(START_CHUNCK, "").replace(END_CHUNK, "") == payload) return true;
        return false
    }
    ...
    async #impl_dumb_attack_stub(url, param, delay, cookie=true, headers=null, target){
        ...
        if(payloads_list.length <= 0) return false;
    ...
    async attack_factory(url, param, delay, cookie=true, headers=null, target){
        this.#attack_setting();
        if(!await this.#impl_dumb_attack_stub(url, param, delay, cookie, headers, target)) return false;
    }
}